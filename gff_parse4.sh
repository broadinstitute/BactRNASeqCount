...
######## This script converts refseq and genbank .gff files to feature files that can be used as input for FEATURE_COUNT_COORDS 
######## which assigns read count numbers to each feature.  NOTE: .fna and .gff accessions must match for counts to be assigned 
######## correctly.  This script was built based on .gff files generated prior to 2019.  Parsing .gff files with newer formatting
######## may require modification of this code

##### path to directory containing .fna and .gff files
REF_PATH=$1"/"
shift
##### path to directory into which feature files will be written
OUT_DIR=$1"/"
shift
##### path to directory into which temporary files will be written
TEMP_DIR=$1"/"
shift
##### number of bases to extend the 5' ends of CDSs (i.e. to assign reads in 5'UTRs to CDS features rather to their upstream intergenes)
ADD5=$1
shift
##### number of bases to extend the 3' ends of CDSs (i.e. to assign reads in 3'UTRs to CDS features rather to their downstream intergenes)
ADD3=$1
shift
##### accession number of .gff and .fna files (file names must match ACC.fna and ACC.gff exactly)
ACC=$1
...
############## function to extract and compare accessions in .fna to a .gff accession passed as an argument
fna_extract ()
{
...
############## a function to generate counts for all feature types in parsed .gff
gff_info_parse ()
{
	LINE=$1
	shift
...
do	
	...
	cat $GFF_FILE | tr '\r' '\n' | grep -v "#" | sed -e 's/ /_/g' -e '/^$/d' -e 's/)//g' -e 's/(//g' -e 's/|//g' -e 's/Coding_gene/CDS/g' | sort -k4n  | awk -v REP=$REP '{if($3 !~ /region/ && $3 !~ /ource/ && $1 == REP) print $0}' > $GFF_ACC_FILE
	...
	do
		...
		awk -F"\t" -v REP=$REP -v START=$START -v DIR=$DIR -v TAG="" -v INFO="" -v NL=$NL -v FOUND="N" '{
											...
											{	
												...
												F_TAG=TAG
												F_INFO="type="TAG";full_tag="TAG":"LC":"GENE":"PROD":"ID":"DIR":"LEN";"INFO
												F_ANN=$2
												F_DIR=$7
												F_ACC=$1
												F_MARK=$8
	...
	cat $GENE_ACC_FILE | awk -F"\t" -v last_end=1 -v last_name=="ORIGIN" -v last_tag="ORIGIN" -v last_dir="|" '{
		...
		### check if .gff accession matches .fna accession  
